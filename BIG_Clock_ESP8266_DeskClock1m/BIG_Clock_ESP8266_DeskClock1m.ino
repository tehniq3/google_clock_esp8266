...
v.1.l - changed exterior sensor to DHT22 (insted DS18B20)
v.1.m - changed exterior sensor to AHT25
*/
...
#include "Arduino.h"
//#include "DHTesp.h"               // https://www.arduino.cc/reference/en/libraries/dht-sensor-library-for-espx/
#include <Adafruit_AHTX0.h>       // https://learn.adafruit.com/adafruit-aht20/arduino
...
int realDS = 1;  // no sensors -> 0, real sensor = 1
...
void setup() 
{
  Serial.begin(115200);
  Serial.println("=");
  Serial.println("============niq_ro's Google clock.......v.1.m.....============");
  sensor_inhouse.begin();
//  sensor_outhouse.begin();
  // Autodetect is not working reliable, don't use the following line
  // dht.setup(DHT_pin);
  // use this instead: 
//  dht.setup(DHT_pin, DHTesp::DHT22); // Connect DHT sensor to GPIO 
  if (! aht.begin()) 
  {
...
//  WiFi.begin(ssid, password);
  printStringWithShift("niq_ro's Google clock.......v.1.l................",30);
...
//    temperatureCout = dht.getTemperature();
  humidityout = humidity.relative_humidity;
  temperatureCout = temp.temperature;
...
//    Serial.print("*C / DHT22 outdoor: ");
    Serial.print("*C / AHT25 outdoor: ");
    Serial.print(temperatureCout);
    Serial.print("*C, humidityout; ");
    Serial.print(humidityout);
    Serial.println("%RH ");     
    }
    else
    ...
    String derece3;
    char resultout2[4]; 
    dtostrf(humidityout, 3, 0, resultout2);   
    if (m%2 == 0) 
    {
      derece3 += ", humidity = ";
    }
    ...
    lenghdrc3 = derece3.length();
    char Buf3[lenghdrc3]; //    char Buf[8];
    derece3.toCharArray(Buf3,lenghdrc3); // derece.toCharArray(Buf,8);   
    printStringWithShift(Buf3,50); 
    delay(3000);
